const getData = async () => {
    ...
    try {
        // fetch 
        const bookContent= await fetch('http://localhost:3000/fetchmyboookings');
        const propertyContent= await fetch('http://localhost:3000/mybookingpropertydata');  
        ...
        for (let i = 0; i < bookingData.length; i++) {
            ...
            const card = `<a href="/mybookings/${bookingData[i].bookingId}" class="text-decoration-none text-black"><div class="img">
                <img class="card-img-top" src="./images/${propertyData[i][0].gallery[0][0]}" alt="Card image cap">
            </div>
