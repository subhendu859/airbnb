const getpropertypage = async () => {
    try {
        // fetch 
        const fetchBookData = await fetch(`http://localhost:3000/mybookingsingledata`);
        const fetchPropData = await fetch(`http://localhost:3000/mybookingsinglepropdata`);
        const ratingBtn = await fetch(`http://localhost:3000/mybookingratebutton`);
        const rating = await fetch(`http://localhost:3000/propertyratingmybook`);
        const fetchreview = await fetch(`http://localhost:3000/propertyreviewingmybook`);
        ...
        bigImg.innerHTML = `<img src="../images/${propData[0].gallery[0][0]}"
